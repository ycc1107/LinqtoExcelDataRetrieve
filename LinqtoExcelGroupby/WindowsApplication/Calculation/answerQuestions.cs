...
namespace Testing.Calculation
{
    class answerQuestions
    {
        ...
        public answerQuestions(string fileNameInput)
        {
            ...
            _writePath = folderName + @"\data\TestOutput.csv";
            fileNameInput = fileNameFormat(fileNameInput);
            _readPath = folderName + @"\data\" + fileNameInput;
        ...
        {
            string result;
            if (fileName == default(string) || fileName == "")
                result = "AIFMD.xlsm";
            else
                result = fileName + @".xlsm";
            return result;
        }
